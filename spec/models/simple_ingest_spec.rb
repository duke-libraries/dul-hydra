...
RSpec.describe SimpleIngest, type: :model, batch: true, ingest: true do
  ...
  subject { SimpleIngest.new('batch_user' => user.user_key, 'folder_path' => folder_path) }
  ...
  describe 'validation' do
    before do
      ...
      allow(subject).to receive(:metadata_provider) { simple_ingest_metadata }
      allow(subject).to receive(:filesystem_node_paths) { fs_node_paths }
    end
    describe 'metadata file' do
      describe 'valid locators' do
        before { allow(simple_ingest_metadata).to receive(:locators) { fs_node_paths } }
        it "should be valid" do
          expect(subject).to be_valid
        end
      end
      describe 'invalid locators' do
        before { allow(simple_ingest_metadata).to receive(:locators) { fs_node_paths + [ 'bar' ] } }
