...
module DulHydra
  describe Validations do
    ...
    describe "validating uniqueness" do
      ...
      before(:all) { Validatable.validates_uniqueness_of :title }
      after(:all) { Validatable.clear_validators! }
      context "on a new record" do
        context "when the value is not taken" do
          before { allow(Validatable).to receive(:where).with(:title => "I am Validatable") { [] } }
          it "should be valid" do
            expect(@obj).to be_valid
          end
        end
        context "when the value is taken" do
          before { allow(Validatable).to receive(:where).with(:title => "I am Validatable") { [taken] } }
      ...
      context "on a persisted record" do
        ...
        context "when the value is not taken" do
          before do
            allow(Validatable).to receive(:where).with(:title => "I am Validatable", "-id" => "foobar:1") { [] }
          end
          it "should be valid" do
            expect(@obj).to be_valid
          end
        ...
        context "when the value is taken by another record" do
          before do
            allow(Validatable).to receive(:where).with(:title => "I am Validatable", "-id" => "foobar:1") { [taken] } 
          end
          it "should not be valid" do
            expect(@obj).not_to be_valid
          end
