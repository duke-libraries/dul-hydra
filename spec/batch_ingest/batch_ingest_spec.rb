...
module DulHydra::BatchIngest
  ...
  describe BatchIngest do
    ...
    context "batch ingest" do
      let(:batch_ingest) { BatchIngest.new }
      context "valid ingest" do
        let(:ingest_batch) do
          [ FactoryGirl.build(:test_model_omnibus_ingest_object),
            FactoryGirl.build(:test_model_omnibus_ingest_object),
            FactoryGirl.build(:test_model_omnibus_ingest_object)
          ]
        end
        before do
          ...
          batch_ingest.batch(ingest_batch, File.join(@test_dir, 'batch_ingest.log'))
          @repo_objects = TestModelOmnibus.all
        end
        after do
          @repo_objects.each {|o| o.destroy}
          FileUtils.remove_dir @test_dir
        end
        it "should ingest the objects" do
          expect(@repo_objects.size).to eq(3)
        end
      end
    end
    
    context "object ingest" do
      context "valid ingest" do
        let(:batch_ingest) { BatchIngest.new }
        ...
        context "target object" do
          ...
          before do
            ingest_object.target_for = collection_object.pid
            @repo_object = batch_ingest.ingest(ingest_object)
          end
          after do
            @repo_object.destroy
