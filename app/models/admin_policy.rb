...
class AdminPolicy < Hydra::AdminPolicy
  ...
  DEFAULT_APO_DEFAULT_RIGHTS = [{type: 'group', name: 'public', access: 'read'},
  ...
  APO_DEFAULT_PERMISSIONS = [{type: 'group', name: 'repositoryAdmin', access: 'edit'}]

  # Create the default APO and return it
  def self.create_default_apo!
    apo = AdminPolicy.create(:pid => DEFAULT_APO_PID)
    apo.default_permissions = APO_DEFAULT_PERMISSIONS
    apo.save
    return apo
  end
  ...
  def set_default_permissions
    if self.new_object? && self.permissions.empty?
      self.permissions = APO_DEFAULT_PERMISSIONS
    end
  end
